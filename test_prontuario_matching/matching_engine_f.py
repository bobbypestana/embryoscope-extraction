"""
matching_engine_f.py — Strategy F: Controlled and Optimized Patient Matching with Fuzzy First Name
=============================================================================================
...
Supports Levenshtein-based fuzzy first name match with edit distance <= 1 for first names > 4 characters.
...
def run_strategy_f(
    ...
    """
    Runs Strategy F patient matching on the source table.
    ...
    logger.info("[%s] Starting Strategy F matching", tag)
    ...
    source_con.execute("""
        CREATE OR REPLACE TEMPORARY MACRO score_name(p_w, c_w) AS (
            CASE
                ...
                WHEN p_w = c_w THEN 1
                WHEN NOT (len(list_filter(c_w, w -> levenshtein(w, p_w[1]) <= CASE WHEN len(p_w[1]) <= 4 THEN 0 ELSE 1 END)) > 0) THEN 5
    ...
    total = len(final_df)
    logger.info("[%s] Strategy F: %d/%d matched (%.2f%%) in %.1f seconds",
