...
def main():
    ...
    logger.info(f"Source: gold.pesquisa_dados_para_ia")
    logger.info("=" * 70)

    # Check pesquisa_dados_para_ia totals
    dp_total_rows = conn.execute('SELECT COUNT(*) FROM gold.pesquisa_dados_para_ia').fetchone()[0]
    dp_unique_embryos = conn.execute('SELECT COUNT(DISTINCT "Slide ID") FROM gold.pesquisa_dados_para_ia WHERE "Slide ID" IS NOT NULL').fetchone()[0]
    ...
    # Query from pesquisa_dados_para_ia (the actual source)
    total = dp_unique_embryos

    if total == 0:
        logger.warning("No embryos found in gold.pesquisa_dados_para_ia.")
        conn.close()
        return
    ...
    embryo_desc = conn.execute('SELECT COUNT(DISTINCT "Slide ID") FROM gold.pesquisa_dados_para_ia WHERE "Slide ID" IS NOT NULL AND "Embryo Description" IS NOT NULL').fetchone()[0]
    clinisys_desc = conn.execute('SELECT COUNT(DISTINCT "Slide ID") FROM gold.pesquisa_dados_para_ia WHERE "Slide ID" IS NOT NULL AND "Embryo Description Clinisys" IS NOT NULL').fetchone()[0]
    clinisys_detalhes = conn.execute('SELECT COUNT(DISTINCT "Slide ID") FROM gold.pesquisa_dados_para_ia WHERE "Slide ID" IS NOT NULL AND "Embryo Description Clinisys Detalhes" IS NOT NULL').fetchone()[0]
