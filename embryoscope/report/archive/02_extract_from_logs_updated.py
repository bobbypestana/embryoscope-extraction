"""
Log Extraction Script - Recover Embryo Image Availability Data (Bronze/Silver Compatible)
Parses log files to extract results and save to Bronze/Silver tables.

This script is useful for recovering data from logs if the API check script fails
...
def main():
    ...
    # Save to Bronze and Silver
    logger.info("Saving to Bronze and Silver tables...")
    
    # Get next log_id for Bronze
    ...
    # Insert into Bronze (append-only)
    conn.execute("INSERT INTO bronze.embryo_image_availability_logs SELECT * FROM bronze_df")
    logger.info(f"✓ Saved {len(bronze_df):,} records to Bronze")
    
    # Upsert to Silver (delete existing + insert new)
    embryo_ids = final_df['embryo_EmbryoID'].tolist()
    placeholders = ','.join(['?' for _ in embryo_ids])
    conn.execute(f"DELETE FROM silver.embryo_image_availability_latest WHERE embryo_EmbryoID IN ({placeholders})", embryo_ids)
    conn.execute("INSERT INTO silver.embryo_image_availability_latest SELECT * FROM final_df")
    logger.info(f"✓ Upserted {len(final_df):,} records to Silver")
