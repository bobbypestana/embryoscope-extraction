...
LOG_PATH = os.path.join(LOGS_DIR, f'silver_loader_ultra_robust_{timestamp}.log')
logging.basicConfig(
    level=logging.DEBUG,
...
def safe_date_parse(date_col):
    """Generate SQL for safe date parsing that handles invalid dates"""
    return f"""
    CASE 
        WHEN {date_col} IS NULL OR LENGTH({date_col}) != 10 OR {date_col}[3] != '/' OR {date_col}[6] != '/' THEN NULL
        WHEN NOT REGEXP_MATCHES({date_col}, '^[0-9]{{2}}/[0-9]{{2}}/[0-9]{{4}}$') THEN NULL
        WHEN CAST(SUBSTR({date_col}, 1, 2) AS INTEGER) < 1 OR CAST(SUBSTR({date_col}, 1, 2) AS INTEGER) > 31 THEN NULL
        WHEN CAST(SUBSTR({date_col}, 4, 2) AS INTEGER) < 1 OR CAST(SUBSTR({date_col}, 4, 2) AS INTEGER) > 12 THEN NULL
        WHEN CAST(SUBSTR({date_col}, 7, 4) AS INTEGER) < 1900 OR CAST(SUBSTR({date_col}, 7, 4) AS INTEGER) > 2100 THEN NULL
        WHEN CAST(SUBSTR({date_col}, 4, 2) AS INTEGER) = 2 AND CAST(SUBSTR({date_col}, 1, 2) AS INTEGER) > 29 THEN NULL
        WHEN CAST(SUBSTR({date_col}, 4, 2) AS INTEGER) IN (4, 6, 9, 11) AND CAST(SUBSTR({date_col}, 1, 2) AS INTEGER) > 30 THEN NULL
        WHEN CAST(SUBSTR({date_col}, 4, 2) AS INTEGER) = 2 AND CAST(SUBSTR({date_col}, 1, 2) AS INTEGER) = 29 AND 
             NOT (CAST(SUBSTR({date_col}, 7, 4) AS INTEGER) % 4 = 0 AND 
                  (CAST(SUBSTR({date_col}, 7, 4) AS INTEGER) % 100 != 0 OR CAST(SUBSTR({date_col}, 7, 4) AS INTEGER) % 400 = 0)) THEN NULL
        ELSE TRY_CAST(STRPTIME({date_col}, '%d/%m/%Y') AS DATE)
    END"""

def safe_time_parse(time_col):
    """Generate SQL for safe time parsing"""
    return f"""
    CASE 
        WHEN {time_col} IS NULL OR NOT REGEXP_MATCHES({time_col}, '^[0-9]{{2}}:[0-9]{{2}}$') THEN NULL
        WHEN CAST(SUBSTR({time_col}, 1, 2) AS INTEGER) < 0 OR CAST(SUBSTR({time_col}, 1, 2) AS INTEGER) > 23 THEN NULL
        WHEN CAST(SUBSTR({time_col}, 4, 2) AS INTEGER) < 0 OR CAST(SUBSTR({time_col}, 4, 2) AS INTEGER) > 59 THEN NULL
        ELSE TRY_CAST(STRPTIME({time_col}, '%H:%M') AS TIME)
    END"""
...
table_casts = {
    ...
    'view_medicamentos_prescricoes': f'''
        ...
        {safe_date_parse('data_inicial')} AS data_inicial,
        {safe_date_parse('data_final')} AS data_final,
        {safe_time_parse('hora')} AS hora,
    ...
    'view_congelamentos_embrioes': f'''
        ...
        {safe_date_parse('responsavel_recebimento_data')} AS responsavel_recebimento_data,
        TRY_CAST(responsavel_armazenamento AS DOUBLE) AS responsavel_armazenamento,
        {safe_date_parse('responsavel_armazenamento_data')} AS responsavel_armazenamento_data,
    ...
    'view_descongelamentos_embrioes': f'''
        ...
        {safe_date_parse('DataCongelamento')} AS DataCongelamento,
        {safe_date_parse('DataDescongelamento')} AS DataDescongelamento,
        CAST(Ciclo AS VARCHAR) AS Ciclo,
        CAST(Identificacao AS VARCHAR) AS Identificacao,
        CAST(CodCongelamento AS VARCHAR) AS CodCongelamento,
        CAST(Tambor AS VARCHAR) AS Tambor,
        ...
        {safe_date_parse('DataTransferencia')} AS DataTransferencia,
    ...
    'view_extrato_atendimentos_central': f'''
        ...
        {safe_date_parse('data_agendamento_original')} AS data_agendamento_original,
    ...
    'view_orcamentos': f'''
        ...
        {safe_date_parse('data_entrega')} AS data_entrega,
        ...
        {safe_date_parse('data_pagamento')} AS data_pagamento,
        CAST(descricao_pagamento AS VARCHAR) AS descricao_pagamento,
        {safe_date_parse('data')} AS data,
        CAST(responsavel AS VARCHAR) AS responsavel,
        {safe_date_parse('data_entrega_orcamento')} AS data_entrega_orcamento,
        {safe_date_parse('data_ultima_modificacao')} AS data_ultima_modificacao,
        hash,
        STRPTIME(extraction_timestamp, '%Y%m%d_%H%M%S') AS extraction_timestamp
    ''',
...
def main():
    logger.info('Starting ultra-robust silver loader')
    ...
    logger.info('Ultra-robust silver loader finished successfully')
