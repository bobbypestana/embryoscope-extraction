...
LOG_PATH = os.path.join(LOGS_DIR, f'silver_loader_robust_{timestamp}.log')
logging.basicConfig(
    level=logging.DEBUG,
...
table_casts = {
    ...
    'view_congelamentos_embrioes': '''
        ...
        CASE WHEN Data_clean IS NOT NULL THEN TRY_CAST(STRPTIME(Data_clean, '%d/%m/%Y') AS DATE) ELSE NULL END AS Data,
        CASE WHEN Hora_clean IS NOT NULL THEN TRY_CAST(STRPTIME(Hora_clean, '%H:%M') AS TIME) ELSE NULL END AS Hora,
        ...
        CASE WHEN responsavel_recebimento_data_clean IS NOT NULL THEN TRY_CAST(STRPTIME(responsavel_recebimento_data_clean, '%d/%m/%Y') AS DATE) ELSE NULL END AS responsavel_recebimento_data,
        TRY_CAST(responsavel_armazenamento AS DOUBLE) AS responsavel_armazenamento,
        CASE WHEN responsavel_armazenamento_data_clean IS NOT NULL THEN TRY_CAST(STRPTIME(responsavel_armazenamento_data_clean, '%d/%m/%Y') AS DATE) ELSE NULL END AS responsavel_armazenamento_data,
    ...
    'view_descongelamentos_embrioes': '''
        ...
        CASE WHEN DataCongelamento_clean IS NOT NULL THEN TRY_CAST(STRPTIME(DataCongelamento_clean, '%d/%m/%Y') AS DATE) ELSE NULL END AS DataCongelamento,
        CASE WHEN DataDescongelamento_clean IS NOT NULL THEN TRY_CAST(STRPTIME(DataDescongelamento_clean, '%d/%m/%Y') AS DATE) ELSE NULL END AS DataDescongelamento,
        CAST(Ciclo AS VARCHAR) AS Ciclo,
        CAST(Identificacao AS VARCHAR) AS Identificacao,
        CAST(CodCongelamento AS VARCHAR) AS CodCongelamento,
        CAST(Tambor AS VARCHAR) AS Tambor,
        ...
        CASE WHEN DataTransferencia_clean IS NOT NULL THEN TRY_CAST(STRPTIME(DataTransferencia_clean, '%d/%m/%Y') AS DATE) ELSE NULL END AS DataTransferencia,
    ...
    'view_extrato_atendimentos_central': '''
        ...
        CASE WHEN data_agendamento_original_clean IS NOT NULL THEN TRY_CAST(STRPTIME(data_agendamento_original_clean, '%d/%m/%Y') AS DATE) ELSE NULL END AS data_agendamento_original,
    ...
    'view_orcamentos': '''
        ...
        CASE WHEN data_entrega_clean IS NOT NULL THEN TRY_CAST(STRPTIME(data_entrega_clean, '%d/%m/%Y') AS DATE) ELSE NULL END AS data_entrega,
        ...
        CASE WHEN data_pagamento_clean IS NOT NULL THEN TRY_CAST(STRPTIME(data_pagamento_clean, '%d/%m/%Y') AS DATE) ELSE NULL END AS data_pagamento,
        CAST(descricao_pagamento AS VARCHAR) AS descricao_pagamento,
        CASE WHEN data_clean IS NOT NULL THEN TRY_CAST(STRPTIME(data_clean, '%d/%m/%Y') AS DATE) ELSE NULL END AS data,
        CAST(responsavel AS VARCHAR) AS responsavel,
        CASE WHEN data_entrega_orcamento_clean IS NOT NULL THEN TRY_CAST(STRPTIME(data_entrega_orcamento_clean, '%d/%m/%Y') AS DATE) ELSE NULL END AS data_entrega_orcamento,
        CASE WHEN data_ultima_modificacao_clean IS NOT NULL THEN TRY_CAST(STRPTIME(data_ultima_modificacao_clean, '%d/%m/%Y') AS DATE) ELSE NULL END AS data_ultima_modificacao,
        hash,
        STRPTIME(extraction_timestamp, '%Y%m%d_%H%M%S') AS extraction_timestamp
    ''',
...
def main():
    logger.info('Starting robust silver loader')
    ...
    logger.info('Robust silver loader finished successfully')
