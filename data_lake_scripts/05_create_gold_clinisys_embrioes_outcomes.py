...
1. Comprehensive Grain: Includes ALL embryos (~320,000) from gold.clinisys_embrioes with an explicit `is_transferred` flag.
2. Independent Dual Matching: Matches Planilha and REDLARA independently across all transferred embryos.
3. Preserved Raw Sources: Preserves all individual source fields (planilha_*, redlara_*, clinisys_*) side-by-side.
4. Quality Cross-Check: Flags concordances, discordances, and source coverage (crosscheck_status, has_conflict, conflict_type).
5. Clean Procedure Filtering: Filters out non-transfer rows (REVITRI, EMBRYO VITRI, CANCELLATION, etc.) to prevent false positives.
"""
...
def create_gold_clinisys_embrioes_outcomes(con):
    logger.info("=" * 90)
    logger.info("BUILDING GOLD.CLINISYS_EMBRIOES_OUTCOMES")
    logger.info("=" * 90)
    
    t0 = time.time()
    
    # 1. Prepare Base Transferred Embryos CTE
    ...
    SELECT 
        ...
        CASE 
            WHEN (c.emb_cong_transferidos = 'Transferido' OR c.descong_em_DataTransferencia IS NOT NULL OR c.trat2_data_transferencia IS NOT NULL) THEN 1
            WHEN (c.trat1_data_transferencia IS NOT NULL 
        ...
        CASE 
            WHEN (c.emb_cong_transferidos = 'Transferido' OR c.descong_em_DataTransferencia IS NOT NULL OR c.trat2_data_transferencia IS NOT NULL) THEN 'FET'
            WHEN (c.trat1_data_transferencia IS NOT NULL 
                  AND (c.trat1_motivo_nao_transferir IS NULL OR c.trat1_motivo_nao_transferir = '')
    ...
    logger.info(f"Identified {transf_count:,d} transferred embryos in Clinisys.")
    ...
    # 4. Independent Matching 1: Transferred Embryos -> Planilha (4 Sheets Waterfall)
    ...
    WITH 
    p1_fet AS (
        SELECT c.oocito_id, f.source_type, f.result, f.tipo_do_resultado, f.gravidez_clinica, f.gravidez_bioquimica, f.no_nascidos, f.data_parto, '1. FET: Exact Transfer Date' as plan_rule
    ...
    p1_recep AS (
        SELECT c.oocito_id, r.source_type, r.result, r.tipo_do_resultado, r.gravidez_clinica, r.gravidez_bioquimica, r.no_nascidos, r.data_parto, '2. RECEP: Exact Transfer Date' as plan_rule
    ...
    p2_fresh AS (
        SELECT c.oocito_id, fr.source_type, fr.result, fr.tipo_do_resultado, fr.gravidez_clinica, fr.gravidez_bioquimica, fr.no_nascidos, fr.data_parto, '3. FRESH: Exact Puncture Date' as plan_rule
    ...
    p2_fot AS (
        SELECT c.oocito_id, fot.source_type, fot.result, fot.tipo_do_resultado, fot.gravidez_clinica, fot.gravidez_bioquimica, fot.no_nascidos, fot.data_parto, '4. FOT: Exact Proc/Cryo Date' as plan_rule
        FROM tmp_transferred_scope c JOIN tmp_fot_clean fot ON c.prontuario = fot.prontuario AND (c.data_puncao = fot.proc_date OR c.fresh_transfer_date = fot.proc_date OR c.data_crio = fot.crio_date)
    ...
    p3_fet_crio AS (
        SELECT c.oocito_id, f.source_type, f.result, f.tipo_do_resultado, f.gravidez_clinica, f.gravidez_bioquimica, f.no_nascidos, f.data_parto, '5. FET: Exact Cryo Date' as plan_rule
    ...
    p3_recep_crio AS (
        SELECT c.oocito_id, r.source_type, r.result, r.tipo_do_resultado, r.gravidez_clinica, r.gravidez_bioquimica, r.no_nascidos, r.data_parto, '6. RECEP: Exact Cryo Date' as plan_rule
        FROM tmp_transferred_scope c JOIN tmp_recep_clean r ON c.prontuario = r.prontuario AND c.data_crio = r.crio_date
        WHERE c.is_transferred = 1 AND c.transfer_category = 'FET' AND c.data_crio IS NOT NULL
    ...
    p4_fet_win AS (
        SELECT c.oocito_id, f.source_type, f.result, f.tipo_do_resultado, f.gravidez_clinica, f.gravidez_bioquimica, f.no_nascidos, f.data_parto, '7. FET: Date Window (+/- 2d)' as plan_rule
        ...
        WHERE c.is_transferred = 1 AND c.transfer_category = 'FET' AND c.fet_transfer_date IS NOT NULL
          AND c.oocito_id NOT IN (SELECT oocito_id FROM p1_fet UNION ALL SELECT oocito_id FROM p1_recep UNION ALL SELECT oocito_id FROM p2_fresh UNION ALL SELECT oocito_id FROM p2_fot UNION ALL SELECT oocito_id FROM p3_fet_crio UNION ALL SELECT oocito_id FROM p3_recep_crio)
    ),
    p4_fresh_win AS (
        SELECT c.oocito_id, fr.source_type, fr.result, fr.tipo_do_resultado, fr.gravidez_clinica, fr.gravidez_bioquimica, fr.no_nascidos, fr.data_parto, '8. FRESH: Puncture Window (+/- 2d)' as plan_rule
        ...
        WHERE c.is_transferred = 1 AND c.transfer_category = 'FRESH' AND c.data_puncao IS NOT NULL
          AND c.oocito_id NOT IN (SELECT oocito_id FROM p1_fet UNION ALL SELECT oocito_id FROM p1_recep UNION ALL SELECT oocito_id FROM p2_fresh UNION ALL SELECT oocito_id FROM p2_fot UNION ALL SELECT oocito_id FROM p3_fet_crio UNION ALL SELECT oocito_id FROM p3_recep_crio UNION ALL SELECT oocito_id FROM p4_fet_win)
    )
    SELECT * FROM p1_fet UNION ALL SELECT * FROM p1_recep UNION ALL SELECT * FROM p2_fresh UNION ALL SELECT * FROM p2_fot
    UNION ALL SELECT * FROM p3_fet_crio UNION ALL SELECT * FROM p3_recep_crio UNION ALL SELECT * FROM p4_fet_win UNION ALL SELECT * FROM p4_fresh_win;
    """)

    # 5. Independent Matching 2: ALL Transferred Embryos -> REDLARA
    ...
    WITH 
    r1_transf AS (
        SELECT c.oocito_id, r.source_type, r.redlara_outcome, r.redlara_outcome_type, r.redlara_gravidez_clinica, r.redlara_gravidez_bioquimica, r.redlara_no_nascidos, r.redlara_data_parto, r.redlara_tipo_parto, '1. REDLARA: Exact Transfer Date' as redlara_rule
    ...
    r2_crio AS (
        SELECT c.oocito_id, r.source_type, r.redlara_outcome, r.redlara_outcome_type, r.redlara_gravidez_clinica, r.redlara_gravidez_bioquimica, r.redlara_no_nascidos, r.redlara_data_parto, r.redlara_tipo_parto, '2. REDLARA: Exact Cryo Date' as redlara_rule
        FROM tmp_transferred_scope c JOIN tmp_redlara_clean r ON c.prontuario = r.prontuario AND c.data_crio = r.crio_date
        WHERE c.is_transferred = 1 AND c.data_crio IS NOT NULL AND c.oocito_id NOT IN (SELECT oocito_id FROM r1_transf)
    ),
    r3_win AS (
        SELECT c.oocito_id, r.source_type, r.redlara_outcome, r.redlara_outcome_type, r.redlara_gravidez_clinica, r.redlara_gravidez_bioquimica, r.redlara_no_nascidos, r.redlara_data_parto, r.redlara_tipo_parto, '3. REDLARA: Transfer Window (+/- 2d)' as redlara_rule
        ...
        WHERE c.is_transferred = 1 AND c.effective_transfer_date IS NOT NULL AND c.oocito_id NOT IN (SELECT oocito_id FROM r1_transf UNION ALL SELECT oocito_id FROM r2_crio)
    )
    ...
    logger.info("Step 5: Materializing gold.clinisys_embrioes_outcomes...")
    ...
    logger.info(f"Total rows in gold table: {total_rows:,d}")
